                                           ...
                                           int num_buffers )
{
  ...
  IOWR_32DIRECT( lcd->base, 4, ( int )display->buffer_ptrs[display->buffer_being_displayed]->buffer ); /* Where our frame buffer starts */
...
int alt_video_display_buffer_is_available( alt_video_display* display )
{
  ...
  if(( IORD_32DIRECT( display->controller_base, 0xC ) == (int)display->buffer_ptrs[display->buffer_being_written]->buffer ) &&
...
void tpo_lcd_isr (void* context, alt_u32 id) 
{
    ...
    ( display->num_frame_buffers == 1 ))
  {
  	...
  	alt_tpo_lcd_set_display_buffer( display, display->buffer_ptrs[next_disp_buf]->buffer );
  }
