...
//                                          arialroundedmtbold_58,
      ...
      while( alt_video_display_buffer_is_available( display ));
      CopyBlockToNextFrame( 36, 56, 210, 140, display );
      alt_video_display_register_written_buffer( display );
...
//    {
      ...
      alt_video_display_register_written_buffer( display );
      while( alt_video_display_buffer_is_available( display ));
      CopyBlockToNextFrame( playlist_gui->loc_x, playlist_gui->loc_y, 
                            playlist_gui->scroll.loc_x + playlist_gui->scroll.size_x, 
                            playlist_gui->loc_y + playlist_gui->size_y,
                            display );
...
/*****************************************************************************
 *  Function: DisplayPlaylistButtons
 *
 *  Purpose:
 * 
 *  Returns: 0
 ****************************************************************************/
/*
int DisplayPlaylistButtons( player_struct* player,
                            alt_video_display* display )
{
  int i;
  ...
  {
    while ( alt_video_display_buffer_is_available( display ) != 0 );
    DisplayButton( &player->gui->gui_main_playlist.edit_button, -1, -1, display );
    DisplayButton( &player->gui->gui_main_playlist.random_button, -1, -1, display );
    DisplayButton( &player->gui->gui_main_playlist.repeat_button, -1, -1, display );
    alt_video_display_register_written_buffer( display );
  }
}
*/
 ...
 *  Returns: 0
 ****************************************************************************/
int UpdatePlaylistButtons( player_struct* player,
                           int touchscreen_x,
                           int touchscreen_y,
                           int pen_is_down,
                           alt_video_display* display )
{
  ...
  UpdateButton( display, touchscreen_x, touchscreen_y, pen_is_down, 
  ...
  if( gui->gui_main_playlist.edit_button.pen_up_event_occured )
    ret_code = GO_EDIT_PLAYLIST_SCREEN;
...
//                                display );    
    ...
    // Display Buttons
//    DisplayMainButtons( gui_main_buttons, display );
...
/*****************************************************************************
 *  Function: DrawInitialScreenPlaylist
 *
 *  Purpose:
 * 
 *  Returns: 0
 ****************************************************************************/
void DrawInitialScreenPlaylist( gui_playlist_buttons_struct* gui_playlist_buttons, 
                                alt_video_display* display )
{
  int i;
  ...
  {
//    alt_video_display_clear_screen( display, 0x0 );
    DrawWindow( 0, 0, 800, 478, "Edit Playlist", display );      
    alt_video_display_register_written_buffer( display );
  }
}
...
int UpdateGUIMain( player_struct* player )
{
  ...
  {                             
    ...
    ret_code = UpdatePlaylistButtons( player, touchscreen_x, touchscreen_y, 
                                      pen_is_down, display_global );
//    if( ret_code == GO_EDIT_PLAYLIST_SCREEN )
//    {
//      player->gui->active_screen = PLAYLIST_SCREEN_ACTIVE;
//      DrawInitialScreenPlaylist( &gui->gui_playlist_buttons, display_global );
//    }
 ...
 ****************************************************************************/
/*
int UpdateGUIPlaylist( player_struct* player )
{
...
//  int i;
  ...
  {
    ticks_at_last_pen_touch = alt_nticks();
    player->gui->active_screen = MAIN_SCREEN_ACTIVE;
    DrawInitialScreenMain( &gui->gui_main_buttons, display_global );
    gui->needs_update = 1;
    UpdateGUIMain( player );
    DisplayPlaylistButtons( player, display_global );
    DisplayMainButtons( player, display_global );
    gui->needs_update = 0;
  }
 ...
 *  Returns: 0
 ****************************************************************************/
int UpdateGUI( player_struct* player )
{
  int ret_code;
  if( player->gui->active_screen == MAIN_SCREEN_ACTIVE )
  {
    ret_code = UpdateGUIMain( player );
  }
/*
  else if( player->gui->active_screen == PLAYLIST_SCREEN_ACTIVE )
  {
    ret_code = UpdateGUIPlaylist( player );
  }
*/  
  return( ret_code );
}
