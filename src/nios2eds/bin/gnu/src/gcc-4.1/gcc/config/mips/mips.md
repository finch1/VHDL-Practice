...
;; flag_pic because they first load the target address into a register.
(define_attr "jal_macro" "no,yes"
  (cond [(eq_attr "jal" "direct")
	 (symbol_ref "TARGET_CALL_CLOBBERED_GP || flag_pic")
...
(define_attr "cpu"
  "r3000,4kc,4kp,5kc,5kf,20kc,24k,24kx,74kc,74kf,74kx,m4k,r3900,r6000,r4000,r4100,r4111,r4120,r4130,r4300,r4600,r4650,r5000,r5400,r5500,r7000,r8000,r9000,sb1,sb1a,sr71000,octeon,xlr"
...
;; This mode macro is for all integer modes smaller than DImode.
(define_mode_macro SUBDI [QI HI SI])
...
;; This code macro allows both sorts of extraction to be treated alike.
(define_code_macro any_extract [sign_extract zero_extract])

;; This code macro allows the three shift instructions to be generated
;; from the same template.
(define_code_macro any_shift [ashift ashiftrt lshiftrt])

;; This code macro allows both of the right shift codes to be treated alike.
(define_code_macro any_shiftrt [ashiftrt lshiftrt])
...
;; This code macros allows both equality operators to be treated alike.
(define_code_macro equality_op [eq ne])
	 ...
	 (subreg:QI
	  (truncate:SI
	   (plus:DI (match_operand:DI 1 "register_operand" "d")
		    (match_operand:DI 2 "register_operand" "d"))) 3)))]
  "TARGET_64BIT && ISA_HAS_BADDU && TARGET_BIG_ENDIAN"
  "baddu\\t%0,%1,%2"
  [(set_attr "type" "arith")])
...
(define_insn "*trunc_zero_ext_<SHORT:mode><GPR:mode>"
  [(set (match_operand:GPR 0 "register_operand" "=d")
        (zero_extend:GPR (truncate:SHORT
  ...
  "!TARGET_MIPS16"
{
  if (mips_expand_unaligned_load (operands[0], operands[1],
    ...
    {
      mips_emit_register_ext (SIGN_EXTRACT, operands);
      DONE;
    }
  else
    FAIL;
  ...
  "!TARGET_MIPS16"
{
  if (mips_expand_unaligned_load (operands[0], operands[1],
				  ...
				  INTVAL (operands[3])))
    DONE;
  else if (mips_use_ext_p (operands))
    {
      mips_emit_register_ext (ZERO_EXTRACT, operands);
      DONE;
    }
  else
    FAIL;
  ...
  "!TARGET_MIPS16"
{
  if (mips_expand_unaligned_store (operands[0], operands[3],
				   ...
				   INTVAL (operands[2])))
    DONE;
  else if (mips_use_ins_p (operands))
    {
...
(define_insn "*insv<mode>di"
  ...
  "TARGET_64BIT && mips_use_ins_p (operands)"
  "dins\t%0,%3,%2,%1"
  [(set_attr "type" "shift")
   (set_attr "mode" "DI")])
...
(define_insn "*insv_<code>_<mode>di"
  ...
  "TARGET_64BIT
   && ISA_HAS_EXT_INS
   && INTVAL (operands[1]) <= <topbit> + 1
   && mips_use_ins_p (operands)"
  "dins\t%0,%3,%2,%1"
  [(set_attr "type" "shift")
   (set_attr "mode" "DI")])

(define_insn "*insvdi_clear_upper32"
  [(set (zero_extract:DI (match_operand:DI 0 "register_operand" "+d")
                         (match_operand 1 "const_int_operand" "")
                         (match_operand 2 "const_int_operand" ""))
