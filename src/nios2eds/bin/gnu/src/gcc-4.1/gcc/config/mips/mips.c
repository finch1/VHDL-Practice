...
mips_emit_scc (enum rtx_code code, rtx target)
{
  ...
  if (code == EQ || code == NE)
    {
      rtx scc_operand = mips_get_seq_sne_operand (mode, op1);
      if (scc_operand)
	mips_emit_binary (code, target, op0, scc_operand);
      else
	{
    ...
    mips_emit_int_relational (code, 0, target, op0, op1);
  return true;
}
...
/* Return true if bit number BIT of X is not the most significant,
   but must nevertheless be a sign bit.

   On 64-bit targets, we must preserve the invariant that, when a
   subword value is stored in a register, the upper 32 bits are a
   sign-extension of the lower 32 bits.  Thus if we set the paradoxial
   DImode subreg of a subword REG, the upper 33 bits of the destination
   must be treated as sign bits.

   ??? This function exists for testing the destination of extzv and
   insv patterns.  If the topmost bit of the destination is such a sign
   bit, a dext or dins instruction may not be used, as the instruction
   would not preserve the required invariant.

   For most operations we'd treat this sort of situation as a constraint
   violation; TRULY_NOOP_TRUNCATION should prevent the middle-end from
   using such SUBREGs in cases where they might be harmful.  However,
   extzv and insv are special cases, because they do not have separate
   named patterns for each mode; instead, their interfaces unconditionally
   promote all register operands to word_mode.  */

static bool
mips_lower_sign_bit_p (rtx x, HOST_WIDE_INT bit)
{
  return (bit >= 31
...
/* Return true if the given insv operands are suitable for the
   "ins" or "dins" instruction.  */

bool
mips_use_ins_p (rtx *operands)
{
  return (mips_use_ins_ext_p (operands[0], operands[1], operands[2])
	  && !mips_lower_sign_bit_p (operands[0],
				     INTVAL (operands[1])
				     + INTVAL (operands[2])
				     - 1));
}

/* Return true if the given extzv operands are suitable for the
   "ext" or "dext" instruction.  */

bool
mips_use_ext_p (rtx *operands)
{
  return mips_use_ins_ext_p (operands[1], operands[2], operands[3]);
}

/* Emit a register ext or extv instruction with operands OPERANDS.
   CODE is SIGN_EXTRACT for ext and ZERO_EXTRACT for extv.  */

void
mips_emit_register_ext (enum rtx_code code, rtx *operands)
{
  rtx dest, src, length, lsb;
  ...
  /* An ugly wart in the interface of these expanders.  It isn't in
     general valid to assign to a paradoxical (subreg:M (foo:N)) if:

       !TRULY_NOOP_TRUNCATION (GET_MODE_BITSIZE (M), GET_MODE_BITSIZE (N))

     However, extzv and insv are special cases, because they do not have
     separate named patterns for each mode; instead, their interfaces
     unconditionally promote all register operands to word_mode.  */
  if (TARGET_64BIT
      && GET_CODE (dest) == SUBREG
      && GET_MODE (dest) == word_mode
      && GET_MODE_CLASS (GET_MODE (SUBREG_REG (dest))) == MODE_INT
      && GET_MODE_SIZE (GET_MODE (SUBREG_REG (dest))) < UNITS_PER_WORD)
    {
      if (INTVAL (length) <= 31)
	/* The value we're extracting is smaller than 31 bits, so using
	   exts or dins will leave the upper 33 bits the same, giving a
	   properly-extended SImode value.  We have combined extract-and-
	   truncate patterns for exactly this case.  */
	src = gen_rtx_fmt_eee (code, word_mode, src, length, lsb);
      else if (lsb == const0_rtx)
	/* This amounts to a truncation from SRC to DEST.  */
	src = src;
      else
	/* This amounts to an arithmetic right shift by >= 32 bits.
	   We have shift-and-truncate insns for exactly this case.  */
	src = gen_rtx_ASHIFTRT (word_mode, src, lsb);

      dest = SUBREG_REG (dest);
      src = gen_rtx_TRUNCATE (GET_MODE (dest), src);
    }
  else
    src = gen_rtx_fmt_eee (code, GET_MODE (dest), src, length, lsb);
...
override_options (void)
{
  ...
  mips_char_to_class['c'] = (TARGET_USE_PIC_FN_ADDR_REG ? PIC_FN_ADDR_REG :
...
print_operand (FILE *file, rtx op, int letter)
{
  ...
  if (letter == 'C')
    ...
    switch (code)
      ...
      default:
	output_operand_lossage ("invalid bit condition");
      }
...
mips_emit_loadgp (void)
{
  ...
  if (TARGET_RTP_PIC && cfun->machine->global_pointer > 0)
    {
      rtx base, index;
...
mips_output_function_prologue (FILE *file, HOST_WIDE_INT size ATTRIBUTE_UNUSED)
{
  ...
  if (TARGET_ABICALLS && TARGET_OLDABI && cfun->machine->global_pointer > 0)
    {
...
/* Output assembly instructions to peform a conditional branch.
   ...
   specified by the third contant operand is set or clear in the
   second operand.
