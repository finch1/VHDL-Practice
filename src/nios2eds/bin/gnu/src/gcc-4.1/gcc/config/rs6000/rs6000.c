...
static bool rs6000_reg_live_or_pic_offset_p (rs6000_stack_t *, int);
...
static bool
rs6000_reg_live_or_pic_offset_p (rs6000_stack_t *info, int reg)
{
...
rs6000_emit_prologue (void)
{
  ...
  if (!WORLD_SAVE_P (info) && using_store_multiple)
           ...
           && info->first_gp_reg_save != 32)
    {
      ...
      for (i = 0; i < 32 - info->first_gp_reg_save; i++)
        if (rs6000_reg_live_or_pic_offset_p (info, info->first_gp_reg_save + i))
          {
            rtx reg = gen_rtx_REG (reg_mode, info->first_gp_reg_save + i);
            rtx offset, addr, mem;
